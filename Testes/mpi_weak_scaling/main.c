...
void execute_with_size(int maze_size, int num_procs, int rank, int size) {
    char maze[maze_size][maze_size];
    if (rank == 0) {
        createMaze(maze_size); // Cria o labirinto no processo raiz
    }
    ...
    if (rank == 0) {
        printf("Processos: %d, Maze Size: %d, Tempo: %f segundos\n", num_procs, maze_size, end - start);
    }
}
...
int main(int argc, char **argv) {
    ...
    for (int maze_size = 50, num_procs = 1; maze_size <= 200; maze_size *= 2, num_procs *= 2) {
        execute_with_size(maze_size, num_procs, rank, size);
    }
